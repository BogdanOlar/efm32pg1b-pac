...
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("CTRL")
            .field("wdogrmode", &format_args!("{}", self.wdogrmode().bits()))
            .field(
                "lockuprmode",
                &format_args!("{}", self.lockuprmode().bits()),
            )
            .field("sysrmode", &format_args!("{}", self.sysrmode().bits()))
            .field("pinrmode", &format_args!("{}", self.pinrmode().bits()))
            .field("resetstate", &format_args!("{}", self.resetstate().bits()))
            .finish()
    }
}
...
impl crate::Resettable for CTRL_SPEC {
    const RESET_VALUE: Self::Ux = 0x4224;
}
