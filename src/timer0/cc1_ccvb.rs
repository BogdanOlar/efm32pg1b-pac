///Register `CC1_CCVB` reader
pub type R = crate::R<CC1_CCVBrs>;
///Register `CC1_CCVB` writer
pub type W = crate::W<CC1_CCVBrs>;
///Field `CCVB` reader - CC Channel Value Buffer
pub type CcvbR = crate::FieldReader<u16>;
///Field `CCVB` writer - CC Channel Value Buffer
pub type CcvbW<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
impl R {
    ///Bits 0:15 - CC Channel Value Buffer
    #[inline(always)]
    pub fn ccvb(&self) -> CcvbR {
        CcvbR::new((self.bits & 0xffff) as u16)
    }
}
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("CC1_CCVB")
            .field("ccvb", &self.ccvb())
            .finish()
    }
}
impl W {
    ///Bits 0:15 - CC Channel Value Buffer
    #[inline(always)]
    pub fn ccvb(&mut self) -> CcvbW<'_, CC1_CCVBrs> {
        CcvbW::new(self, 0)
    }
}
///CC Channel Buffer Register
///
///You can [`read`](crate::Reg::read) this register and get [`cc1_ccvb::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`cc1_ccvb::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).
pub struct CC1_CCVBrs;
impl crate::RegisterSpec for CC1_CCVBrs {
    type Ux = u32;
}
///`read()` method returns [`cc1_ccvb::R`](R) reader structure
impl crate::Readable for CC1_CCVBrs {}
///`write(|w| ..)` method takes [`cc1_ccvb::W`](W) writer structure
impl crate::Writable for CC1_CCVBrs {
    type Safety = crate::Unsafe;
}
///`reset()` method sets CC1_CCVB to value 0
impl crate::Resettable for CC1_CCVBrs {}
